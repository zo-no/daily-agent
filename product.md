...
### LN-084 admission: account-bound Agent MCP/Skill bridge

- **Core-loop contribution:** improves `browse → search → edit/delete` by letting a paired Codex or Claude session locate the current account's bounded plans and records, prepare one explicit change, and verify the result without adding a required step to ordinary quick recording.
- **Evidence:** on 2026-09-05 the product owner explicitly requested three separate capabilities: Agent plan authoring, Agent record authoring, and Google Calendar synchronization. The existing in-page Diary/Plan Agent and isolated Mastra Tool do not provide an external, account-bound MCP surface, so a distinct bridge is required.
- **Default cost:** the capability is secondary and lives in Account settings plus a local stdio MCP process. Pairing is a one-time local action; every plan or record write adds a visible proposal review and explicit confirmation. The home page, quick-record composer, required fields, and normal manual save path do not change.
- **Offline, privacy and recovery:** the first bridge keeps the authenticated browser as the account-state owner and forwards only bounded, allowlisted plan, record, and category data through loopback. It never exposes Supabase service keys, Google tokens, passwords, complete account documents, image Blobs, or another account's data. Proposal, confirmation, and pairing state are transient and excluded from backups. Confirmed changes reuse the existing local-first `commitData` and revision/CAS path; stale, expired, cancelled, revoked, offline, or failed operations make no write and do not weaken manual offline use. Raw record text changes only when the user explicitly confirms that exact field change.
- **Verification and removability:** focused protocol, controller, queue, browser, and stdio handshake regressions cover bounded reads, proposal-only behavior, explicit confirmation, CRUD, stale revision, idempotency, account replacement, revocation, and read-back. The full quality gate remains required. Real Codex/Claude discovery, real account pairing, and multi-account lifecycle evidence are tracked separately and must remain open until performed. Removing the bridge, Skill, local command, and settings panel requires no data migration.
- **Exit condition:** keep the bridge isolated or remove it if a real client cannot discover it without unsafe credentials, if confirmation cost exceeds manual editing, if any cross-account leak or silent overwrite occurs, or if account, offline, backup, accessibility, or quality-gate behavior regresses during the observation window.
- **Admission score:** `18/20`, mainline candidate behind a secondary local bridge. The capability is not considered accepted until the real-client and real-account evidence is recorded in `PROJECT_BOARD.md`.
