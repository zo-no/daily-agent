...
import { CALENDAR_DIARY_SCHEMA_VERSION, CALENDAR_REVIEW_KINDS, MAX_CALENDAR_REVIEW_ENTRIES, MAX_CALENDAR_REVIEW_EVENTS, MAX_CALENDAR_REVIEW_SUGGESTIONS, normalizeCalendarDiaryReviewOutput, sanitizeCalendarDiaryReviewInput, validateCalendarDiaryReviewResponse } from "./model.mjs";
...
const entrySchema = z.object({ id: z.string().min(1).max(128), time: z.string().max(8), content: z.string().min(1).max(4000) }).strict();
...
export async function reviewCalendarDiaryWithDeepSeek(input, { apiKey = process.env.DEEPSEEK_API_KEY, baseUrl = process.env.DEEPSEEK_BASE_URL || "https://api.deepseek.com", fetchImpl = globalThis.fetch, model = process.env.DEEPSEEK_MODEL || "deepseek-chat", now = Date.now, timeoutMs = AI_TIMEOUT_MS } = {}) {
  try {
    ...
    throw toDeepSeekRouteError(caught, {
      invalidOutput: { code: "AI_CALENDAR_DIARY_REVIEW_RESPONSE_INVALID", message: "calendar diary review request failed" },
      unavailable: { code: "AI_UNAVAILABLE", message: "calendar diary review request failed" },
