...
import { z } from "zod";
import { AI_TIMEOUT_MS, AiClassifierError, MAX_AI_CONTENT_CHARS, MAX_AI_ENTRIES, bearerToken, errorResponse, hasAllowedOrigin, hasJsonContentType, jsonResponse, readJsonBody } from "../../../shared/ai/http-boundary.mjs";
...
import { MAX_DOMAIN_DAILY_OVERVIEW_CHARS, MAX_DOMAIN_DAILY_THEMES, MAX_DOMAIN_DAILY_THEME_SUMMARY_CHARS, MAX_DOMAIN_DAILY_THEME_TITLE_CHARS, normalizeDomainDailySummaryOutput, sanitizeDomainDailySummaryInput, validateDomainDailySummaryResponse } from "./model.mjs";
...
export const domainDailySummaryInputSchema = z.object({ domainName: z.string().min(1).max(80), date: z.string().length(10), locale: z.enum(["en", "zh-CN"]), entries: z.array(z.object({ id: z.string().min(1).max(128), date: z.string().length(10), time: z.string().max(8), content: z.string().max(MAX_AI_CONTENT_CHARS), sourceType: z.enum(["ordinary", "periodic"]) }).strict()).min(1).max(MAX_AI_ENTRIES) }).strict();
...
export async function summarizeDomainTodayWithDeepSeek(input, { apiKey = process.env.DEEPSEEK_API_KEY, baseUrl = process.env.DEEPSEEK_BASE_URL || "https://api.deepseek.com", fetchImpl = globalThis.fetch, model = process.env.DEEPSEEK_MODEL || "deepseek-chat", now = Date.now, timeoutMs = AI_TIMEOUT_MS } = {}) {
  try {
    ...
    throw toDeepSeekRouteError(caught, {
      invalidOutput: { code: "AI_DOMAIN_DAILY_SUMMARY_RESPONSE_INVALID", message: "daily domain summary request failed" },
      unavailable: { code: "AI_UNAVAILABLE", message: "daily domain summary request failed" },
      sharedMessage: "daily domain summary request failed"
    });
  }
}
