  ...
  async function request(mode, value) {
    try {
      const result = await postRemoteAiJson({
        endpoint,
        input: minimalPayload(value, mode),
        fetchImpl,
        getAccessToken,
        signal: value.signal,
      ...
      const fallbackReason = ["aborted", "timeout"].includes(error?.code) ? "remote-timeout" : "remote-unavailable";
      if (mode === "reply") return { ...localReply(value), fallbackReason };
