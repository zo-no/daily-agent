...
import { setGoalRecordAssociation } from "@/lib/goal-model.mjs";
...
export function GoalDetailPage({ goalId }) {
  ...
  const { data, commitData, hydrated } = useLogNoteData();
  ...
  function associateRecord(recordId, keyResultId) {
    if (!goal) return;
    const hasAssociations = Boolean(goal.recordIds?.length || goal.keyResults?.some((item) => item.recordIds?.length));
    const isGoalOnly = keyResultId === "__goal__";
    const next = setGoalRecordAssociation(hasAssociations ? goal : { ...goal, recordIds: facts.evidence.map((item) => item.id) }, recordId, isGoalOnly ? null : keyResultId || null, Boolean(keyResultId));
    commitData((state) => ({ ...state, goals: state.goals.map((item) => item.id === goal.id ? { ...next, updatedAt: Date.now() } : item) }));
  }
  ...
  return <main className="goal-detail-page">
    ...
    <div className="goal-detail-shell">
      <header className="goal-detail-hero"><span className="goal-detail-kicker">O · {t("goals.objective")}</span><span className={`goal-status is-${goal.status}`}>{t(`goals.status.${goal.status}`)}</span><h2>{goal.content}</h2><p>{facts.goal.startDate && facts.goal.endDate ? `${facts.goal.startDate} → ${facts.goal.endDate}` : t("goals.undated")}</p></header>
      ...
      <section className="goal-detail-section"><div className="goal-section-heading"><span>K</span><h2>{t("goals.keyResults")}</h2></div>{!facts.keyResults.length ? <p className="goal-muted">{t("goals.noKeyResults")}</p> : <div className="goal-detail-krs">{facts.keyResults.map((item) => { const value = percent(item.progress); return <article key={item.id}><div><span className={`goal-status is-${item.status}`}>{t(`goals.status.${item.status}`)}</span><h3>{item.content}</h3><p>{value === null ? t("goals.qualitativeProgress", { count: item.evidenceCount }) : t("goals.numericProgress", { current: item.currentValue, target: item.targetValue, unit: item.unit || "", percent: value })}</p><p className="goal-kr-evidence">{t("goals.krEvidenceCount", { count: item.evidenceCount })}</p></div>{value !== null && <div className="goal-progress" aria-label={`${value}%`}><span style={{ width: `${value}%` }} /></div>}</article>; })}</div>}</section>
      <section className="goal-detail-section"><div className="goal-section-heading"><span>R</span><h2>{t("goals.timeline")}</h2></div>{!grouped.length ? <div className="goal-detail-empty"><p>{facts.periodDays.length ? t("goals.noEvidence") : t("goals.undatedEvidence")}</p></div> : <div className="goal-timeline">{grouped.map((group) => <section key={group.date}><h3>{group.date}</h3>{group.entries.map((entry) => <article key={entry.id}><time>{entry.time || t("entry.noTime")}</time><div className="goal-evidence-content"><p>{entry.content}</p>{entry.keyResultIds.length > 0 && <div className="goal-evidence-badges" aria-label={t("goals.associatedKeyResults")}>{entry.keyResultIds.map((keyResultId) => <span key={keyResultId}>{facts.keyResults.find((item) => item.id === keyResultId)?.content || keyResultId}</span>)}</div>}<label className="goal-evidence-link"><span>{t("goals.associateWith")}</span><select value={entry.keyResultIds[0] || (goal.recordIds?.includes(entry.id) ? "__goal__" : "")} onChange={(event) => associateRecord(entry.id, event.target.value)} aria-label={t("goals.associateRecord", { content: entry.content.slice(0, 40) })}><option value="">{t("goals.unassigned")}</option><option value="__goal__">{t("goals.goalOnly")}</option>{facts.keyResults.map((item) => <option key={item.id} value={item.id}>{item.content}</option>)}</select></label></div></article>)}</section>)}</div>}</section>
