...
/**
 * @fileoverview 共享的目标/OKR 模块：作为首页内联模块与独立 `/goals` 路由的同一逻辑来源。
 * 只负责渲染与目标 CRUD，状态由 useLogNoteData 提供；不带 ManagementHeader 或页面级外壳。
 */
...
export function GoalsWorkspace({ embedded = false }) {
  const { t } = useI18n();
  ...
  if (!hydrated) return <div className="goals-loading">{t("goals.loading")}</div>;

  return (
    <div className="goals-workspace" data-goals-workspace>
      <div className="goals-toolbar">
        {!embedded && <p className="goals-intro">{t("goals.intro")}</p>}
        <button className="goals-add" type="button" onClick={() => setDraft(createGoalDraft())}>
          <Icon name="plus" />{t("goals.add")}
        </button>
      </div>
      {!goals.length && !draft && (
        <section className="goals-empty"><Icon name="plan" size={30} /><h2>{t("goals.emptyTitle")}</h2><p>{t("goals.emptyHint")}</p></section>
      )}
      <div className="goals-list">
        {goals.map((goal) => <article className="goal-card" key={goal.id}>
          <div className="goal-card-main">
            <span className="goal-card-kicker">O · {t("goals.objective")}</span>
            <span className={`goal-status is-${goal.status}`}>{t(`goals.status.${goal.status}`)}</span>
            <h2>{goal.content}</h2>
            {(goal.startDate || goal.endDate) && <p>{goal.startDate || "…"} → {goal.endDate || "…"}</p>}
            <div className="goal-card-krs">
              <span className="goal-card-krs-label">K</span>
              {goal.keyResults?.length
                ? <div><strong>{t("goals.keyResultCount", { count: goal.keyResults.length })}</strong><ul>{goal.keyResults.slice(0, 3).map((item) => <li key={item.id}>{item.content}</li>)}</ul>{goal.keyResults.length > 3 && <small>{t("goals.moreKeyResults", { count: goal.keyResults.length - 3 })}</small>}</div>
                : <span className="goal-card-empty">{t("goals.noKeyResults")}</span>}
            </div>
            <p className="goal-card-evidence">{t("goals.evidenceSummary", { evidence: goalFacts.get(goal.id)?.metrics.evidenceCount || 0, days: goalFacts.get(goal.id)?.metrics.recordedDayCount || 0 })}</p>
          </div>
          <div className="goal-card-actions">
            <Link href={`/goals/${encodeURIComponent(goal.id)}`}>{t("goals.review")}</Link>
            <button type="button" onClick={() => setDraft({ ...goal, keyResults: (goal.keyResults || []).map((item) => ({ ...item })) })}>{t("common.edit")}</button>
            <button type="button" onClick={() => deleteGoal(goal)}>{t("common.delete")}</button>
          </div>
        </article>)}
      </div>
      {draft && <section className="goal-editor"><form onSubmit={saveGoal}>
        <div className="goal-editor-header"><h2>{draft.id ? t("goals.edit") : t("goals.add")}</h2><button type="button" onClick={() => setDraft(null)} aria-label={t("common.close")}><Icon name="close" /></button></div>
        <label><span>{t("goals.content")}</span><input autoFocus value={draft.content} onChange={(event) => setDraft({ ...draft, content: event.target.value })} maxLength={240} /></label>
