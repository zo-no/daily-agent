...
import { AiClassifierError, MAX_AI_CONTENT_CHARS, MAX_AI_ENTRIES } from "./ai-route-boundary.mjs";
...
function validTime(value) { if (value === "") return true; const m = /^(\d{2}):(\d{2})$/.exec(String(value)); return Boolean(m) && Number(m[1]) < 24 && Number(m[2]) < 60; }
