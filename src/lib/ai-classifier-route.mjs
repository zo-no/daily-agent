...
export async function analyzeWithDeepSeek(input, {
  ...
  try {
    ...
    if (failure === "invalid-output") throw new AiClassifierError("AI_RESPONSE_INVALID", "model returned invalid classification JSON", 502);
    if (failure === "upstream-error") throw new AiClassifierError("AI_UPSTREAM_ERROR", "model request failed", 502);
