  ...
  sections: Object.freeze({
    ...
    "zh-CN": Object.freeze([
      ...
      section("optional-ai", "4. 可选 AI 功能", [
        ...
        "AI 结果会经过校验并默认只在当前会话中存在；只有你明确应用建议或新增内容时才会写入。当前领域的今日总结范围更窄：只发送今天所选领域的记录（最多 80 条，每条正文最多 4,000 个 Unicode 字符），排除计划和其他领域，并且始终只保留在当前会话；不会保存、导出、备份或应用。内容优化中的“使用优化稿”只改变尚未保存的编辑器草稿，仍需点击原有“完成”才会保存。原始记录不会被静默改写。当前配置的 AI 提供方会按其自己的服务和隐私条款处理提交文字。DeepSeek 当前公开的隐私材料说明，服务输入可能用于开发、改进或训练其技术，并说明了退出训练的权利；数据转移后，Log Note 无法控制提供方的处理。请不要提交你不希望该提供方处理的内容。"
      ]),
      section("use", "5. 信息用途", [
