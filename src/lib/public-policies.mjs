  ...
  sections: Object.freeze({
    en: Object.freeze([
      ...
      section("optional-ai", "4. Optional AI features", [
        ...
        "Depending on the tool, this may include selected record or plan text, dates and times, existing category names, limited conflict context, and the messages you type in that review. For content improvement, Log Note sends the exact current free-text draft you chose, its language, and opaque request/fingerprint bindings; it does not include category, tags, attachments, date, time, record ID, or account identity. When you deliberately start Plan review, the title and time of overlapping Google events may be included as read-only conflict context. The separate Calendar and diary review requires a visible human approval, then sends at most 40 current-day event titles/times/all-day markers and 80 current-day diary times/texts under request-local opaque IDs. It excludes real calendar/event/record IDs, links, locations, attendees, tags, categories, attachments, plans, and the complete document. Account identifiers, passwords, Google access tokens, and image files are not included in the AI prompt.",
        "AI results are validated and remain temporary unless you explicitly apply a proposed change or add content. The current-domain daily summary is session-only and sends only today's selected-domain records (up to 80 records, with each text bounded to 4,000 Unicode characters); it is never saved, exported, backed up, or applied. The Calendar and diary review is also always session-only: its suggestions cannot be applied and never change, save, export, or back up records, plans, or Calendar events. For draft improvement, Use improved draft changes only the unsaved editor draft; the existing Done action is still required to save it. Raw notes are never silently rewritten. The configured AI provider processes submitted text under its own service and privacy terms. DeepSeek's published privacy materials currently state that service input may be used to develop, improve, or train its technology and describe a training opt-out; Log Note does not control the provider's processing after transfer. Do not submit content you do not want processed by that provider."
      ]),
      ...
      section("sharing-limited-use", "6. Sharing, sale, and Google Limited Use", [
        ...
        "Only Plan review and the separately approved Calendar and diary review send the bounded Google event context described above to the configured AI provider. Log Note itself does not use the context for advertising, credit decisions, surveillance, or model training, and other AI tools do not receive Google Calendar data. The provider's processing remains governed by its own terms as disclosed above.",
    ...
    "zh-CN": Object.freeze([
      ...
      section("optional-ai", "4. 可选 AI 功能", [
        ...
        "根据不同工具，发送内容可能包括选中的记录或计划文字、日期和时间、已有分类名称、有限的冲突上下文，以及你在该次回顾中输入的消息。内容优化只发送你主动选择的当前这份普通自由文本草稿、语言，以及不含业务身份的随机请求标识和正文 fingerprint；不会发送分类、标签、附件、日期、时间、记录 ID 或账号标识。当你主动开始计划回顾时，发生重叠的 Google 事件标题和时间可能作为只读冲突上下文一并发送。单独的日历与日记复盘会先要求可见的人工确认，再发送最多 40 项当天日程的标题/时间/全天标记与 80 条当天日记的时间/正文，并使用请求内匿名 ID；不会发送真实日历/事件/记录 ID、链接、地点、参与者、标签、分类、附件、计划或完整文档。账号标识、密码、Google access token 和图片文件不会进入 AI 提示词。",
        "AI 结果会经过校验并默认只在当前会话中存在；只有你明确应用建议或新增内容时才会写入。当前领域的今日总结始终只在会话中存在，只发送今天所选领域的记录（最多 80 条，每条正文最多 4,000 个 Unicode 字符）；不会保存、导出、备份或应用。日历与日记复盘同样始终只在会话中存在：建议不能应用，也不会改动、保存、导出或备份记录、计划和日历事件。内容优化中的“使用优化稿”只改变尚未保存的编辑器草稿，仍需点击原有“完成”才会保存。原始记录不会被静默改写。当前配置的 AI 提供方会按其自己的服务和隐私条款处理提交文字。DeepSeek 当前公开的隐私材料说明，服务输入可能用于开发、改进或训练其技术，并说明了退出训练的权利；数据转移后，Log Note 无法控制提供方的处理。请不要提交你不希望该提供方处理的内容。"
      ]),
      section("use", "5. 信息用途", [
      ...
      section("sharing-limited-use", "6. 共享、出售与 Google 有限使用", [
        ...
        "只有计划回顾和另行人工确认的日历与日记复盘会把上文所述、有明确上限的 Google 事件上下文发送给当前 AI 提供方。Log Note 自身不会将这些内容用于广告、信贷判断、监控或模型训练；其他 AI 工具不会接收 Google 日历数据。提供方的处理仍受上文所述其自身条款约束。",
